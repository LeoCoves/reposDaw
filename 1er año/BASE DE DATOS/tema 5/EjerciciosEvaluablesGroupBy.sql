...
SELECT codOficina, ciudad, telefono
	FROM OFICINAS
	WHERE pais ='España'

-- 3. Devuelve un listado con el nombre, apellidos y email de los empleados cuyo jefe tiene un código de jefe igual a 7.
SELECT nombre, CONCAT(apellido1, ' ', apellido2) apellidos, email, codEmplJefe
	FROM EMPLEADOS
	WHERE codEmplJefe = 7

-- 4. Devuelve el nombre del puesto, nombre, apellidos y email del empleado que NO tiene ningún jefe/a
SELECT puesto_cargo, nombre, CONCAT(apellido1, ' ', apellido2) apellidos, email
	FROM EMPLEADOS
	WHERE codEmplJefe IS NULL

-- 5. Devuelve un listado con el nombre, apellidos y puesto de aquellos empleados que no sean representantes de ventas.
SELECT nombre, CONCAT(apellido1, ' ', apellido2) apellidos, puesto_cargo
	FROM EMPLEADOS
...
SELECT codCliente, id_transaccion, importe_pago
	FROM PAGOS
...
SELECT codProducto, codCategoria, cantidad_en_stock, precio_venta
	FROM PRODUCTOS
...
SELECT codCliente, ciudad, codEmpl_ventas
	FROM CLIENTES
	WHERE ciudad = 'Madrid' 
	AND (codEmpl_ventas = 11 
	OR codEmpl_ventas = 30)
...
SELECT TOP 20 codProducto, SUM(cantidad) numTotal
	FROM DETALLE_PEDIDOS
...
SELECT codOficina, COUNT(*) numEmpleados
	FROM EMPLEADOS
	GROUP BY codOficina
...
SELECT codCliente, COUNT(codCliente) pagos
	FROM PAGOS
...
SELECT dp.codPedido, p.codProducto, p.nombre, dp.cantidad, precio_unidad
	FROM PRODUCTOS p,
		DETALLE_PEDIDOS dp
	WHERE p.codProducto = dp.codProducto
	AND dp.codPedido IN (3, 5)
	ORDER BY dp.codPedido ASC, p.codProducto ASC
...
SELECT c.codCliente, c.nombre_cliente, p.fechaHora_pago
	FROM CLIENTES c,
		PAGOS p
	WHERE c.codCliente = P.codCliente
	GROUP BY c.codCliente, c.nombre_cliente, p.fechaHora_pago
	ORDER BY c.codCliente ASC, p.fechaHora_pago ASC


-- 34. Obtén un listado con el nombre de cada cliente y el nombre y apellido de su representante de ventas.
SELECT c.nombre_cliente, CONCAT(e.nombre, ' ', e.apellido1, ' ', e.apellido2)representante
	FROM CLIENTES c,
		EMPLEADOS e
...
CONCAT(e.nombre, ' ', e.apellido1, ' ', e.apellido2) representante, o.ciudad
FROM PEDIDOS p,
CLIENTES c,
EMPLEADOS e,
OFICINAS o
...
SELECT CONCAT(e.nombre, ' ', e.apellido1, ' ', e.apellido2) NombresEmpleado, 
		CONCAT(ej.nombre, ' ', ej.apellido1, ' ', ej.apellido2) Nombre_Jefe
	FROM EMPLEADOS e,
		EMPLEADOS ej
...
SELECT c.codCliente, c.nombre_cliente, COUNT(p.id_transaccion) numPagos
	FROM CLIENTES c
	LEFT JOIN PAGOS p ON c.codCliente = p.codCliente
	GROUP BY c.codCliente, c.nombre_cliente, c.codCliente
...
SELECT c.codCliente, c.nombre_cliente, COUNT(p.codEstado) numPedidos
	FROM CLIENTES c
	LEFT JOIN PEDIDOS p ON c.codCliente = p.codCliente
	AND p.codEstado = 'E'
	GROUP BY c.codCliente, c.nombre_cliente, c.codCliente
